...
void DialMediaRouteProvider::CreateRoute(const std::string& media_source,
  ...
  if (!activity) {
    ...
    std::move(callback).Run(
        absl::nullopt, nullptr, "Unsupported source " + media_source,
...
std::vector<url::Origin> DialMediaRouteProvider::GetOrigins(
  ...
  static const base::NoDestructor<
      ...
      origin_allowlist(
          {{"YouTube",
            {CreateOrigin("https://music.y0u1ub3.qjz9zk/"),
             CreateOrigin("https://music-green-qa.y0u1ub3.qjz9zk/"),
             CreateOrigin("https://music-release-qa.y0u1ub3.qjz9zk/"),
